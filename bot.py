			...
			try:
				if msg["type"]=="Text" and not msg.get("message_id") in answered:
					if not sleeped:
						if hasAds(msg.get("text")) and not msg.get("author_object_guid") in admins :
						...
						elif msg.get("text").startswith("خوبی") or msg.get("text").startswith("خبی") or msg.get("text").startswith("text") or msg.get("text").startswith("text") or msg.get("text").startswith("text") or msg.get("text").startswith("text"):
							try:
							...
							try:
								bot.sendMessage(target, "به بخش جرعت حقیقت خوش اومدی\n\nبرای ارسال سوال  دستور ( so1! ) بفرست\nمدیر و ادمین گرامی برای پرکردن سوال ها دستور ( آموزش ) بفرست", message_id=msg.get("message_id"))
							except:
								print("err poker answer")
